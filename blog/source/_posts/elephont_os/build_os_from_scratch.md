...
![](build_os_from_scratch/37b098d4862d7a43add0bec5b06a1517.png)
...
![](build_os_from_scratch/097e7aff3618649eebf5f81e0d7e9ced.png)
...
![](build_os_from_scratch/45b6c2304dcca12c52c58d947b01ca3c.png)
...
![](build_os_from_scratch/e2d7c6c0e3c71c7454445146b6548696.png)
...
![](build_os_from_scratch/9f17c9fdd145275b0c5904baf690418b.png)
...
![](build_os_from_scratch/b095a420671212006e159bb9739c867d.png)
...
此外，为了内存对齐，进而使得指令执行效率更高，我们想要把数据部分放在内核加载器loader的前0x300个字节，这意味这loader的指令真正开始的位置是物理地址$0x900+0x300=0xb00$ 处。之前我们在loader的开头加了一句jmp语句，这是因为mbr跳转到loader后，loader的开头是一堆数据定义，而不是指令，所以加上这条jmp是有必要的。但是现在，既然我们能知道loader的指令起始地址是0xb00，那么我们直接在修改MBR中的跳转指令`jmp LOADER_BASE_ADDR `为`jmp LOADER_BASE_ADDR + 0x300`，这样就可以省略到loader开头的jmp指令（占3个字节），从而使得在它之后定义的数据的地址均对齐到偶数（具体可见P.183）。
...
![](build_os_from_scratch/d98572826027a229b5816230d3ba996d.png)
...
![](build_os_from_scratch/28cf6d86a5f377a1d626ce20bf055a1a.png)
...
![](build_os_from_scratch/27e4e49d6d8bcc32d79f1f6aaed46acd.png)
...
![](build_os_from_scratch/b20f8d8de36bc20c03f62922ce2f1674.png)
...
![](build_os_from_scratch/3259b5c3e592e2b0d400bdfa380f63a8.png)

执行`s`进入中断，下一个指令正是中断处理程序的`push ds`指令

![](build_os_from_scratch/09b5015dc34ec4de3a1972873e89c0f3.png)

![](build_os_from_scratch/f25ade9724394173f85374fb47eed0f9.png)
...
![](build_os_from_scratch/5814cd347178d2582dea2beb011bb474.png)
...
![](build_os_from_scratch/d42302c5d9fee2dd10f457f413e7e232.png)
...
![](build_os_from_scratch/406577233e43f6acdd8584a1a2e9f15e.png)
...
![](build_os_from_scratch/d8d1812933c9234ec0f1015104fb63a0.png)
...
![](build_os_from_scratch/aa27a8f140e41f6f08ab9a75b2a42046.png)
...
![](build_os_from_scratch/afaf892391da95cc1ddf72d93356f2af.png)
...
![](build_os_from_scratch/dd36fbb3d0ebff89059a1c7bec359c2d.png)
...
![](build_os_from_scratch/0e02d82e8bbe29470d52a141a4b23820.png)
...
![](build_os_from_scratch/b1437d5d6e38abf210eb245f11e3c13b.png)

# 13. 硬盘驱动程序
## 13.1. 给文件系统安个窝，创建一个从盘
![](build_os_from_scratch/6f55b26498682f710d9494e04a5b3102.png)

在配置文件bochsrc.disk中写入：

![](build_os_from_scratch/3ba2c15e6d49de2da4dc37ee7b6aaaa5.png)
...
![](build_os_from_scratch/bc35c664767624d47dde6ab9e70a116a.png)

用t命令设置分区id之后（设置了一个不存在于已知文件系统id集合的值，因此显示unknown。分区id表示文件系统类型，也就是说这些分区上的文件类型是未知的）：

![](build_os_from_scratch/680e3da3eb0d8160856abfb243d182ff.png)

3. 列出分区表

![](build_os_from_scratch/6f239d7586a739f212d6746dbb3e4db9.png)

分区后的硬盘大小：

![](build_os_from_scratch/96dd1f00829a8e8e53e20e70d9c62f00.png)
...
![](build_os_from_scratch/8e3d2af656f1ac44a4151bd7d536eecc.png)
...
![](build_os_from_scratch/de6ddfff0c22e9f37c10c773dffdd451.png)

运行结果如下：

![](build_os_from_scratch/7bbcf77f609644a7ba19992d942e5e53.png)
...
![](build_os_from_scratch/ac5176371e5a4f47e98bb8b9b1c3ce21.png)
...
![](build_os_from_scratch/00f0460e686b213fa5bf4a396d997ed4.png)
...
![](build_os_from_scratch/69d704dfbf4dad646c152f854100a2b6.png)

修改之后，去掉用的printk语句，运行程序，结果如下：
分区表中的内容正好对应了用fdisk工具为从盘hd80M.img划分分区之后，用`fdisk -l hd80M.img`查看分区输出的信息

![](build_os_from_scratch/8a8efd8d404dd26511ab5ff1f60b659e.png)
...
![](build_os_from_scratch/d0249bbfeb8c3127aea31d0dcf445159.png)

再次运行：

![](build_os_from_scratch/ce9592913845dee9b062c7c7ae971431.png)
...
![](build_os_from_scratch/3c0e1baed5fb6d9397c41fddfcde7a8f.png)
...
![](build_os_from_scratch/50928b0174bca57fbe4a73953ea1d8fc.png)

咱们目录项的定义是下面这个样子，其中filename占16个字节，enum 类型的f_type可以理解为int类型，占4个字节，所以一个目录项的大小是24个字节。

![](build_os_from_scratch/5b5f9bb9820d1dc46d77600b88eba49a.png)
...
![](build_os_from_scratch/170404db5f8ff0a09c8cdd2f302f26be.png)
...
![](build_os_from_scratch/a10f9363ba70008eac801d318d0446dd.png)

咱们在main函数中调用sys_open在根目录下创建了文件file1。查看挂载的主盘sdb1中的根目录的数据扇区内容，可见file1的目录项已经建立。

![](build_os_from_scratch/493f2b6ff5dd9c019ae291bd969ca772.png)

再次运行，显示文件已存在，符合预期🥳

![](build_os_from_scratch/6ca0d0d7a10104e3e1b20fae3f050de0.png)

## 14.4. 关闭文件，sys_close的实现
关闭文件，就是把打开文件所建立的访问链条（即 fd$\to$ file table entry $\to$ inode）一一关闭。具体来说，sys_close函数收到主调函数传来的文件描述符fd， 然后调用fd_local_2_global函数把fd转为全局文件表的下标global_fd，通过global_fd咱们就可以得到文件对应的inode了，调用inode_close去掉这个inode，并把global_fd对应的文件表项的fd_inode指针置为NULL，这样才能使得函数get_free_slot_in_global_FT认为这个global_fd有效，可以重新拿来用。

运行结果：
![](build_os_from_scratch/54209d878d7bf28669a2cecf0a3859b1.png)
...
![](build_os_from_scratch/7fe64c9f921b0bdef2c039b84a1e3991.png)
...
![](build_os_from_scratch/0238178fb16e869d18df6318bd38629d.png)

再执行一次试试：

![](build_os_from_scratch/6f424bf951d4cf1988be8dd701931560.png)

再检查下inode_table:

![](build_os_from_scratch/df3b3f6ba58373665673175898c6a771.png)

上面这串数字是个啥呢？ 听我慢慢道来：

咱们的inode结构定义如下：

![](build_os_from_scratch/2b0788a7ea2e139c8e875e01001cec2e.png)

list_elem结构体中内含2个指针prev和next，这意味着一整个inode占用的大小为 $6\times4+13\times4=76$ 个字节，因此上面xxd的输出中前76个字节是根目录的inode，与刚哥给的结果相同，刚哥已经分析过这部分了，那我就说说后面的第2个inode吧，第一个双字内容为1，说明该文件的编号是1，check✅。第2个双字内容为0x18，即24字节，因为该文件是普通文件（咱们在file_create给函数create_dir_entry传递是文件类型参数是FT_REGULAR，因为我们的sys_open不支持创建目录文件），所以这24个字节表示file1文件本身的大小，正好对应了2个`hello,world\n`的大小，check✅。再看第5个双字内容，为0xA68，表示`i_blocks[0]`，与我们打印的地址相同 (此时空闲数据块中只分配出去了一个根目录占有的数据块和file1占有的数据块)，check✅。按照file_write函数的逻辑，在首次写入文件时要先分配一个数据块：

![](build_os_from_scratch/0ce4e53c4572a0735b9aeb3655b47e3f.png)
...
![](build_os_from_scratch/5befaf34f38738b0413ec6e090a4c47a.png)

## 14.7. 移动文件读写指针，sys_lseek的实现
本质上就是修改文件结构(`struct file`)中的`fd_pos`成员

![](build_os_from_scratch/ffdc21957fde4ad43e7918aa1a61fea1.png)
...
![](build_os_from_scratch/2606772b39d210ffec64a9c2e1bfeccf.png)
...
![](build_os_from_scratch/27e0be5f32a1bd7405c3ba6939a809c2.png)

2. inode位图
仅分配出一个inode, 说明咱们确实是把file1的inode给回收了

![](build_os_from_scratch/ff49c9b2a6e4ee6bdd9ff1254b4ec45c.png)

3. inode表
`/file1`的inode被抹去，并且根目录的i_size更新为2个目录项大小，即0x30 (48个字节)

![](build_os_from_scratch/15fa48731e32d1870b3f332035ddcf0d.png)

4. 根目录
file1的目录项已被抹去，现在根目录中仅剩最初的`.`和`..`目录

![](build_os_from_scratch/a789d25c556b3662c942425ffbd22fa5.png)

## 14.9. 创建目录， sys_mkdir的实现

![](build_os_from_scratch/dae0df55c061a0284d7010c9a06aae1b.png)

根目录：

dir1对应的在根目录中的目录项显示，dir1的inode编号为`02`，文件类型为FT_DIRECTORY (表现为值`02`)

![](build_os_from_scratch/4681413d93b549c1e3c51e7feb5008fe.png)
...
![](build_os_from_scratch/d2bf4b3470629311cc0ff8f143c42451.png)

下面我们查看这个地址的内容：

可见目录文件dir1的数据块中存在3目录项：`.` , `..`和`subdir1` , 符合预期 QVQ。

![](build_os_from_scratch/0cbea9d21366cd316848262b4709a1bc.png)
...
![](build_os_from_scratch/0e3428e4a2b69a559391d7ef4d8afcc8.png)

再根据inode表中的内容，查看file2的内容：
这里的字符串内容是向我最喜欢的美剧之一《硅谷》致敬 `^_^`，话说最近OpenAi的董事会开除创始人CEO又马上重新应聘回来的行为真的是跟《硅谷》里面如出一辙啊🤨。

![](build_os_from_scratch/e406b5dab9598c470e3d5d93c0d6951f.png)
## 14.10. 打开和关闭目录，sys_opendir和sys_closedir的实现

![](build_os_from_scratch/ec07045da26f1114a575ef02b4a9f6dd.png)

## 14.11. 遍历目录，sys_readdir和sys_rewinddir的实现

![](build_os_from_scratch/40d3dba8042cbef91306978bbeab28db.png)

## 14.12. 删除空目录，sys_rmdir的实现

![](build_os_from_scratch/f45956d551bb3e3dbc2b056c36e85b9d.png)

## 14.13. 目录项".."展现锋芒， sys_getpwd与sys_chdir的实现
每个目录中存在的目录项`..`终于发挥了它的作用，咱们从当前目录，从下而上的顺藤摸瓜到根目录，就能获取到工作目录的路径啦。

![](build_os_from_scratch/9f5979df5eef713cb375bc0c1b3519f1.png)

## 14.14. 探索sys_stat函数，获取文件属性

![](build_os_from_scratch/2157b95ac191fe426d6945403dfa9d09.png)
...
![](build_os_from_scratch/da49684abe7c8c380e9c6382c153cd4b.png)
...
![](build_os_from_scratch/d326e14a83ef8be61fdddf321e76e41a.png)
...
创建位图
![](build_os_from_scratch/47a5db8ced87e7dc01333df7638e2370.png)

创建内核栈：

![](build_os_from_scratch/b41165a55da34924a923b6e1653ee9eb.png)
...
运行结果：
![](build_os_from_scratch/9b95df32c63ffed1e5284f11a51650f6.png)
...
![](build_os_from_scratch/45f9eab41105200874d150a188a0b356.png)

这里我取的用户名是Peach，也是对应了我最重要的人之一。

运行结果：

![](build_os_from_scratch/6660cb6343807b248af502a9ed7c717c.png)
...
![](build_os_from_scratch/15d0187723549ad1bee964f4589c4a6b.gif)
...
![](build_os_from_scratch/9e65c3959bc3e215922dd5ca70ec9a6e.gif)

### 15.2.4. Shell中的路径解析
这节还是有点难度的，咱们的任务是把用户输入的不规整的路径转换为规整的路径格式，比如说把`/a/b/../c`转变为`/a/c`，这样的规整路径交给`cd`, `ls`等命令，才好处理
![](build_os_from_scratch/20549e368af0766fe7089535df8e0d15.png)
...
![](build_os_from_scratch/d73f4fc97e17a51e5bf9da56d48a25bc.png)

![](build_os_from_scratch/29a1c8717fccd15a7fe5cd3ccb4344ba.png)
...
![](build_os_from_scratch/b12f29b0a452ce9a7473c1e81eee1bbf.png)
...
![](build_os_from_scratch/87abea38b80ec867e8c0d30047737d1f.png)
...
效果如下， 之后咱们就可以用gdb调试啦
![](build_os_from_scratch/d94653838e10729b33d04bf00df07cab.png)

![](build_os_from_scratch/a7dc26009e3b132c5ef6489acc5c11d4.png)
...
![](build_os_from_scratch/d31bd9e08651e4933cedc9b2f88d6f98.png)

在开始加载segment之前，查看文件头中关于程序头表的信息:

![](build_os_from_scratch/6327d5dd909657656027d9ce79164fb4.png)
...
![](build_os_from_scratch/dbeee7e0d7be83e4a0889aa4a4da666b.png)
...
![](build_os_from_scratch/c078758771e6cbd12dcd78e47b6eb3ac.png)

这与`readelf -l`的输出中FileSize字段的值吻合。

接着看看这个segment所处的虚拟页和在虚拟页中的偏移（无偏移）

![](build_os_from_scratch/327f592aa6ebacb78c64001e7d699101.png)
...
![](build_os_from_scratch/3eacc173ae386e0de96c00510af7aa7b.png)

![](build_os_from_scratch/d4a184b0ca3105b66540ca3fa35a70c3.png)

segment1看样子是成功加载了，下面看看segment2：

读取一个32字节的程序头表项之后，此时的文件指针偏移量变成84

![](build_os_from_scratch/179cf558b65a62fc85572ed6c98c61cc.png)

查看segment2所处的虚拟页信息，依旧是页对齐的形式，实际大小`b66`与`readelf -l`的输出相符

![](build_os_from_scratch/199ea69a188b639fe04aa50a6a640782.png)
![](build_os_from_scratch/683eb72f08692531d13959741e03bb87.png)


页目录项和页表项，页表项在segment1的基础上加了一个项的大小（4个字节），这意味着segment2所处的虚拟页紧跟在segment1的虚拟页之后

![](build_os_from_scratch/e42f578cf31a875477b37f3a26f41156.png)
...
![](build_os_from_scratch/d19f329c04a426cf1f778531b29631db.png)

一共分配了6个数据块（编号8~13）, 这个数字怎么来的呢？因为咱实际读取的字节数是`0xb66`，换算为十进制就是2918，一个数据块大小为512个字节，$2918/512$ 向上取整，得到所需数据块数量为6。为了保险，我还是打印了`blocks_required_read`的值，该值表示待读取的数据块数量，！！！这里居然是一个巨大的值，原因很简单，就是无符号数的回绕，因为我用一个小的值`block_read_start_idx`减去了一个大的值`block_read_end_idx`。但其实这里`blocks_required_read`的值对流程的影响并不大，后续分支只是判断它是不是0而已。

![](build_os_from_scratch/531bad7c9536075971a8803fb18a11a1.png)

![](build_os_from_scratch/1b9685ac52b4cdebcdb18108d585d56e.png)

下面开始读入。因为是页对齐的，所以不存在第一个数据块存在旧数据的情况（数据块被完全覆盖），因此第一次读入也是以块为单位

![](build_os_from_scratch/f14d8c4b9960a1bc86ed4ca11c344357.png)

4次循环后查看还未读取的数据量，确实是刚读了2048个字节（$2918-870=2048$）

![](build_os_from_scratch/8889c350a8c0bcf2115eaed1e2ddda3c.png)

读取完size个字节后，进入释放`all_blocks_addr`和`io_buf`的内存阶段。

![](build_os_from_scratch/37b2fee3ccc62e403421a93d71a57405.png)

这2个变量在用户内存池中占有的大小分别是512个字节和560个字节

![](build_os_from_scratch/9c1628c8d8fda13ae55e2f98bd5d3921.png)

先看`sys_free(all_blocks_addr)`，因为是用户进程，所以从用户内存池中释放

![](build_os_from_scratch/a9cce02842372a40c5cc17bd65c1b691.png)
...
![](build_os_from_scratch/267779e9ca1c702ec7f650da869e9b7e.png)
...
![](build_os_from_scratch/5377a8ff484ee7e3e091e2433dd512a5.png)

因此现在`sys_free`进入了回收arena的代码块

![](build_os_from_scratch/43c0510bf2d97461ede8daac5541e15b.png)

对`all_blocks_addr`的释放没有触发页错误，接着我们看对`sys_free(io_buf)`。

执行到这里的时候发生了页错误：

![](build_os_from_scratch/776f3e58261cf6d06883c4dc4a76d8d9.png)

此处应该是512字节大小类型的arena，但其描述符中的信息是乱码

![](build_os_from_scratch/2226d5873e05fbb4ee5e80bf828c765d.png)

回到给io_buf分配内存的sys_malloc语句，进去看看

![](build_os_from_scratch/5fbef48df79c7e7b68160a5b38dbbeef.png)

分配的确实是类型为512字节的arena

![](build_os_from_scratch/9a751bc22e0ee42651c4ea6aac7d8eb9.png)

![](build_os_from_scratch/f7144965f58cf8b809ef2e359b58a410.png)

执行流程没有进入list_empty分支

![](build_os_from_scratch/ca385eb0144e1d98b8fab0c07355a15b.png)
...
![](build_os_from_scratch/47f05e8d1504ddbec3c718ddbc719719.png)

![](build_os_from_scratch/b2cc389e67ee5b9ee0a38e26a70d2f99.png)
...
![](build_os_from_scratch/49ac435e5b8d4f99b6185b623e9673a4.png)

为了搞清楚这个问题，我决定回到fork结束之后，刚进入子进程的时间点，查看此时子进程的块描述符数组`u_mb_desc_arr`。怎么进程PCB的内容全部变成0了😅

![](build_os_from_scratch/10c65bb08a6d1615f23c107cc74b975c.png)

子进程pid居然也为0....., 我现在的情绪是这样的😫。

![](build_os_from_scratch/c2f7d07dd0cf93d91d9de81da8cdec86.png)

那么回顾下内核启动以来调用fork的地方，先是在init进程中调用fork创建了子进程shell，通过ps命令就可以知道，这个fork到没啥问题。

![](build_os_from_scratch/ce67fee9286f5e498370123d7a07c727.png)
...
![](build_os_from_scratch/bd698297e378a4107dcdca764cd17bf2.png)

在函数block_desc_init中检查512类型的arena的描述符的初始化过程：

对于512大小的arena, 一个arena（一页）中可以放7个数据块
![](build_os_from_scratch/ff473a4afaabd08b5386406aa704e932.png)

刚初始化的描述符free_list为空，head指向tail，说明函数block_desc_init初始化的过程没什么问题
![](build_os_from_scratch/0dd41d42586945912754b27bf182373b.png)
...
![](build_os_from_scratch/25f7a7c5ce3bf58beec4c099ccfd3c8f.png)

![](build_os_from_scratch/e68540e2e56051cc576505fe26e3588a.png)
...
![](build_os_from_scratch/c6a0d86144246c1256a4977e9c9e24c5.png)
